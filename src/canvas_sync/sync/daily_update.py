            ...
            if 'Week' in module.name:
                ...
                if week_num:
                    ...
                    for item in items:
                        ...
                        tasks.append(task)
    except (CanvasException, OSError, ValueError) as e:
...
def parse_due_date(value: str) -> datetime | None:
    ...
    try:
        ...
        pass
    try:
        parsed = datetime.strptime(cleaned, "%b %d")
    except ValueError:
        return None
...
def generate_todo_markdown(vault_path: str) -> str:
    ...
    # Sort by due date
    all_tasks.sort(key=lambda x: parse_due_date(x.get('due', '')) or datetime.max)
...
- **SPED 117**: You're in **Group D** — discussions are group-level
